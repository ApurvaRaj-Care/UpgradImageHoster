...
public class UserController {
	...
	public String registerUser(User user, Model model) {
		...
		if (user != null && user.getPassword() != null && checkRegexChar(user.getPassword())
				&& checkRegexDigit(user.getPassword()) && checkRegexSplChar(user.getPassword())) {
	...
	private boolean checkRegexDigit(String checkStr) {
		String regex = "(.)*(\\d)(.)*";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(checkStr);
		System.out.println("checkRegexDigit "+matcher.matches() );
		return matcher.matches();
	}

	private boolean checkRegexChar(String checkStr) {
		String regex = ".*[a-zA-Z]+.*";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(checkStr);
		System.out.println("checkRegexChar "+matcher.matches() );
		return matcher.matches();
	}

	private boolean checkRegexSplChar(String checkStr) {
		String regex = ".*[@#$%^&+=*]+.*";
		Pattern pattern = Pattern.compile(regex);
		Matcher matcher = pattern.matcher(checkStr);
		System.out.println("checkRegexSplChar "+matcher.matches() );
		return matcher.matches();
	}
}
